...
export class Cheer extends AggregateRoot<CheerPrimitives, CheerDomainEvent> {
  ...
  public static create(sport: Sport): Cheer {
    const now = DateUtil.now();

    if (!Object.values(Sport).includes(sport)) {
      throw new DomainException('CHEER', '유효하지 않은 스포츠 종목입니다.', HttpStatus.BAD_REQUEST);
    }

    return new Cheer(sport, 0, 0, now, now);
  }
  ...
  public addLikes(university: University, likes: number): void {
    if (likes < 0) {
      throw new DomainException('CHEER', '좋아요 수는 0 이상이어야 합니다.', HttpStatus.BAD_REQUEST);
    }
    if (!Number.isInteger(likes)) {
      throw new DomainException('CHEER', '좋아요 수는 정수여야 합니다.', HttpStatus.BAD_REQUEST);
    }
