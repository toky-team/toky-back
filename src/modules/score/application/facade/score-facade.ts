...
export class ScoreFacadeImpl extends ScoreFacade {
  ...
  @Transactional()
  async startGame(sport: Sport): Promise<ScorePrimitives> {
  ...
  @Transactional()
  async endGame(sport: Sport): Promise<ScorePrimitives> {
    const score = await this.scoreReader.findBySport(sport);
    if (!score) {
      throw new DomainException('SCORE', '경기 데이터를 찾을 수 없습니다.');
    }
  ...
  @Transactional()
  async updateScore(sport: Sport, kuScore: number, yuScore: number): Promise<ScorePrimitives> {
    const score = await this.scoreReader.findBySport(sport);
    if (!score) {
      throw new DomainException('SCORE', '경기 데이터를 찾을 수 없습니다.');
    }
  ...
  @Transactional()
  async resetScore(sport: Sport): Promise<ScorePrimitives> {
    const score = await this.scoreReader.findBySport(sport);
    if (!score) {
      throw new DomainException('SCORE', '경기 데이터를 찾을 수 없습니다.');
    }
